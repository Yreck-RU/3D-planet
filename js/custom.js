...
function init(){
    ...
    // outer planet
    let outerPlanet = new THREE.Mesh(geomet2, material);
    outerPlanet.scale.x = outerPlanet.scale.y = outerPlanet.scale.z = 0;//10
    skelet.add(outerPlanet);

    // ambient light
    let ambientLight = new THREE.AmbientLight(0x333333);//999999
    scene.add(ambientLight);

    // directional light
    let dLight = [];
    dLight[0] = new THREE.DirectionalLight(0xe4e4dc, 1);//ffffff
    dLight[0].position.set(1, 0, 0);
    dLight[1] = new THREE.DirectionalLight(0xe4e4dc, 1);//00dbde
    dLight[1].position.set(0.75, 1, 0.5);
    dLight[2] = new THREE.DirectionalLight(0xe4e4dc, 1);//fc00ff
